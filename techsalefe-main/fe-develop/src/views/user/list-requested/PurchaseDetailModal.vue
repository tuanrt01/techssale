<template>
  <Modal :show="show" @hidden="closeModal">
    <div class="p-5 transition-all duration-200 ease-in-out">
      ...
      <div class="mt-5">
        ...
        <!-- Timeline Status -->
        <div class="relative pb-5 mt-5 mb-8 min-h-[100px]" v-if="data.status !== 'WAITING'">
          <div class="flex items-center w-full">
            <div class="w-full bg-gray-200 h-1 flex items-center justify-between overflow-x-auto">
              <div v-for="step in filteredStatusSteps" :key="step.status"
...
export default {
  ...
  methods: {
    ...
    isStepPassed(status) {
      ...
      if (this.data.status === 'REJECTED') return false;
      return statusOrder[status] <= statusOrder[this.data.status];
    },
    getStatusDate(status) {
      switch (status) {
        case 'WAITING': return this.data.createdAt;
        case 'PROCESSING': return this.data.updatedAt;
        case 'ORDERING': return this.data.updatedAt;
        case 'SUCCESS': return this.data.completedAt;
        default: return null;
      }
    },
